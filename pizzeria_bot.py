...
class PizzeriaBot:
    ...
    def gestionar_pedido(self, wa_id, texto, inter_id=None):
        try:
            ...
            if (texto.lower() == "hola" and inter_id is None) or inter_id == "hola":
                ...
                # Solo reseteamos si escribió HOLA, si es botón mantenemos total
                if inter_id is None:
            ...
            print(f"DEBUG: {wa_id} en {paso} con ID {inter_id}", flush=True)
            ...
            if paso == "MENU_PRINCIPAL":
                ...
                if inter_id in SALSAS:
                    item = pedido.get("item_en_proceso", "Snack")
                    pedido["extras"].append(f"{item.replace('_',' ').capitalize()} ({inter_id})")
                ...
                if inter_id in TIPOS_PAPAS:
                    item = pedido.get("item_en_proceso", "crazy_papas")
                    pedido["extras"].append(f"Crazy Papas {inter_id}")
                    pedido["total"] += MENU_SNACKS.get(item, 85)
                    pedido.pop("item_en_proceso", None)
                    cliente.paso_actual = "MENU_PRINCIPAL"; cliente.pedido_temporal = json.dumps(pedido); db.session.commit()
                    return self.wa.enviar_botones(wa_id, f"✅ Papas {inter_id} añadidas.\n💰 Total: *${pedido['total']}*", [{"id":"hola","title":"🥤 Ver Menú"},{"id":"pagar","title":"💳 Pagar"}])
    ...
    def _enviar_resumen(self, wa_id, pedido):
        ...
        for i, p in enumerate(pedido["pizzas"]):
            res += f"🍕 *P{i+1} ({p.get('tamano','')}):* {p['nombre']}\n"
    ...
    def _enviar_salsas(self, wa_id):
        ...
        return self.wa.enviar_lista(wa_id, "Salsas", "Elige:", "Mr. Bigo's", "Salsas", [{"title":"Salsas", "rows":rows}])
